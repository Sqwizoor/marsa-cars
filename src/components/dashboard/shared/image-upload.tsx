      ...
      <CldUploadWidget onSuccess={onUpload} uploadPreset="m3jl9tli">
        {({ open }) => (
          <div
            ...
            className={cn(
              "relative rounded-full w-52 h-52 bg-gray-200 border-2 border-white shadow-2xl overflow-visible cursor-pointer transition-all hover:shadow-lg",
              {
                "bg-red-100": error,
          ...
          >
            {value.length > 0 && (
              <Image
                ...
                width={300}
                height={300}
                className="w-52 h-52 rounded-full object-cover absolute inset-0 pointer-events-none"
              />
            )}

            <button
              type="button"
              className="z-50 absolute right-0 bottom-6 flex items-center font-medium text-[17px] h-14 w-14 justify-center text-white bg-gradient-to-t from-blue-primary to-blue-300 border-none shadow-lg rounded-full hover:shadow-md active:shadow-sm hover:scale-105 transition-transform"
              disabled={disabled}
              onClick={(e) => {
                e.stopPropagation();
                if (!disabled) open();
